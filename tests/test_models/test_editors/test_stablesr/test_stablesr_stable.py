...
def test_stable_diffusion():
    ...
    LatentDiffusion.tokenizer = dummy_tokenizer()
    LatentDiffusion.text_encoder = dummy_text_encoder()

    # infer
    with pytest.raises(Exception):
        LatentDiffusion.infer(1, height=64, width=64)

    with pytest.raises(Exception):
        LatentDiffusion.infer('temp', height=31, width=31)

    result = LatentDiffusion.infer(
