...
from diffusers import (AutoencoderKL, ControlNetModel, DiffusionPipeline,
                       UNet2DConditionModel, UniPCMultistepScheduler, logging)
...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> input_image = load_image("https://hf.co/datasets/huggingface
        ...
        >>> pipe_controlnet.scheduler = \
            UniPCMultistepScheduler.from_config(pipe_controlnet.scheduler.config)
        ...
        >>> control_image = load_image(
            ...
            resolve/main/diffusers/vermeer_canny_edged.png")


        >>> result_img = pipe_controlnet(
...
def prepare_controlnet_conditioning_image(controlnet_conditioning_image, width,
                                          height, batch_size,
    ...
    if not isinstance(controlnet_conditioning_image, torch.Tensor):
        ...
        if isinstance(controlnet_conditioning_image[0], PIL.Image.Image):
            ...
            controlnet_conditioning_image = \
                controlnet_conditioning_image.transpose(0, 3, 1, 2)
            controlnet_conditioning_image = \
                torch.from_numpy(controlnet_conditioning_image)
        elif isinstance(controlnet_conditioning_image[0], torch.Tensor):
            controlnet_conditioning_image = \
                torch.cat(controlnet_conditioning_image, dim=0)
    ...
    controlnet_conditioning_image = \
        controlnet_conditioning_image.repeat_interleave(repeat_by, dim=0)
...
class StableDiffusionControlNetImg2ImgPipeline(DiffusionPipeline):
    ...
    def enable_vae_slicing(self):
        ...
        When this option is enabled, the VAE will split the input tensor in
        slices to compute decoding in several
    ...
    def disable_vae_slicing(self):
        r"""
        Disable sliced VAE decoding. If `enable_vae_slicing` was
        previously invoked, this method will go back to
    ...
    def enable_sequential_cpu_offload(self, gpu_id=0):
        r"""
        Offloads all models to CPU using accelerate, significantly
        reducing memory usage. When called, unet,
        text_encoder, vae, controlnet, and safety checker have their
        state dicts saved to CPU and then are moved to a
        `torch.device('meta') and loaded to GPU only when their specific
        submodule has its `forward` method called.
        Note that offloading happens on a submodule basis. Memory savings
    ...
    def enable_model_cpu_offload(self, gpu_id=0):
        r"""
        Offloads all models to CPU using accelerate, reducing memory usage
        with a low impact on performance. Compared
        to `enable_sequential_cpu_offload`, this method moves one whole model
        at a time to the GPU when its `forward`
        method is called, and the model remains in GPU until the next model
        runs. Memory savings are lower than with
        `enable_sequential_cpu_offload`, but performance is much better due to
        the iterative execution of the `unet`.
        """
        if is_accelerate_available() and is_accelerate_version(
                '>=', '0.17.0.dev0'):
            from accelerate import cpu_offload_with_hook
        else:
            raise ImportError('`enable_model_offload` requires' +
                              '`accelerate v0.17.0` or higher.')
    ...
    def _execution_device(self):
        ...
        device can only be inferred from Accelerate's module
        hooks.
        """
    ...
    def _encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            if untruncated_ids.shape[-1] >= text_input_ids.shape[-1] and not \
                    torch.equal(text_input_ids, untruncated_ids):
    ...
    def check_inputs(
        ...
        if (callback_steps is None) or (callback_steps is not None and
            ...
            raise ValueError(f'`callback_steps` has to be a positive integer '
                             f' but is {callback_steps} of type'
        ...
        if (not controlnet_cond_image_is_pil
                and not controlnet_cond_image_is_tensor
                and not controlnet_cond_image_is_pil_list
                and not controlnet_cond_image_is_tensor_list):
            raise TypeError(
                'image must be passed and be one of PIL image, '
                'torch tensor, list of PIL images, or list of torch tensors')

        if controlnet_cond_image_is_pil:
            controlnet_cond_image_batch_size = 1
        elif controlnet_cond_image_is_tensor:
            controlnet_cond_image_batch_size = \
                controlnet_conditioning_image.shape[0]
        ...
        if controlnet_cond_image_batch_size != 1 and \
            ...
            raise ValueError(
                f'If image batch size is not 1, image batch size must '
                f'be same as prompt batch size. image '
                f'batch size: {controlnet_cond_image_batch_size}, '
        ...
        if isinstance(image, torch.Tensor):
            ...
            # if mask_image.ndim != 2 and mask_image.ndim != 3
            #       and mask_image.ndim != 4:
            #     raise ValueError(
            #           "`mask_image` must have 2, 3, or 4 dimensions")

            if image.ndim == 3:
                image_batch_size = 1
                image_channels, image_height, image_width = image.shape
            elif image.ndim == 4:
                image_batch_size, image_channels, image_height, image_width = image.shape  # noqa
        ...
        if self.vae.config.latent_channels != self.unet.config.in_channels:
            raise ValueError(
                f'The config of `pipeline.unet` expects '
                f' {self.unet.config.in_channels} but received'
                f' latent channels: {self.vae.config.latent_channels},'
                f' Please verify the config of `pipeline.unet` and '
                f' the `pipeline.vae`')

        if strength < 0 or strength > 1:
            raise ValueError(f'The value of strength should in'
                             f' [0.0, 1.0] but is {strength}')
    ...
    def __call__(
            ...
            controlnet_conditioning_image: Union[torch.FloatTensor,
                                                 PIL.Image.Image,
                                                 List[torch.FloatTensor],
                                                 List[PIL.Image.Image]] = None,
            strength: float = 0.8,
            ...
            cross_attention_kwargs: Optional[Dict[str, Any]] = None,
            controlnet_conditioning_scale: float = 1.0,
            save_control_frame_ind: int = -1,
            control_save_dir: Optional[str] = None):
        r"""
        Function invoked when calling the pipeline for generation.
        ...
        Args:
            ...
            image (`torch.Tensor` or `PIL.Image.Image`):
                ...
                inpainted, *i.e.* parts of the image will
                be masked out with `mask_image` and repainted according
                to `prompt`.
            controlnet_conditioning_image (`torch.FloatTensor`,
                `PIL.Image.Image`, `List[torch.FloatTensor]` or
            ...
            strength (`float`, *optional*):
                ...
                `image`. Must be between 0 and 1. `image`
                will be used as a starting point, adding more noise to it
                the larger the `strength`. The number of
                denoising steps depends on the amount of noise initially
                added. When `strength` is 1, added noise will
                be maximum and the denoising process will run for the full
                number of iterations specified in
                `num_inference_steps`. A value of 1, therefore, essentially
                ignores `image`.
            height (`int`, *optional*, defaults to
                self.unet.config.sample_size * self.vae_scale_factor):
                The height in pixels of the generated image.
            ...
            negative_prompt (`str` or `List[str]`, *optional*):
                ...
                `negative_prompt_embeds`. instead. If not defined, one has
                to pass `negative_prompt_embeds`. instead.
                Ignored when not using guidance (i.e., ignored if
                `guidance_scale` is less than `1`).
            num_images_per_prompt (`int`, *optional*, defaults to 1):
                The number of images to generate per prompt.
            ...
            cross_attention_kwargs (`dict`, *optional*):
                ...
                the `AttnProcessor` as defined under
                `self.processor` in
        ...
        # If we do sequential model offloading, let's offload
        #  unet and controlnet
        # manually for max memory savings
...
class ControlnetAnimationInferencer(BaseInferencer):
    ...
    def __call__(self,
        ...
        for ind in range(len(all_images)):
            ...
            concat_hed = Image.new('RGB', (image_width * 3, image_height),
                                   'black')
