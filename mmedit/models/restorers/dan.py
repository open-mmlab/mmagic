...
class DAN(BasicRestorer):
    ...
    def forward(self, lq, gt=None, ker_map=None, test_mode=False, **kwargs):
        """Forward function.
        ...
        return self.forward_train(lq, gt, ker_map)

    def forward_train(self, lq, gt, ker_map):
        """Training forward function.

        Args:
            lq (Tensor): LQ Tensor with shape (n, c, h, w).
            gt (Tensor): GT Tensor with shape (n, c, h, w).
            ker_map (Tensor) : ker map Tensor
        Returns:
            Tensor: Output tensor.
        """
        losses = dict()
        srs, fake_kers = self.generator(lq)
        output = srs[-1]
        out_ker = fake_kers[-1]
        loss_pix = 0
        loss_pix += self.pixel_loss(out_ker, ker_map)
    ...
    def forward_dummy(self, GT_img):
        ...
        Returns:
            out (Tensor): Predicted super-resolution results (n, 3, 4h, 4w).
        """
        LR_img, ker_map = self.prepro(GT_img)
        LR_img = (LR_img * 255).round() / 255
        GT_img = GT_img.cuda()
        LR_img = LR_img.cuda()
        ker_map = ker_map.cuda()
        return GT_img, LR_img, ker_map
    ...
    def train_step(self, data_batch, optimizer):
        ...
        """
        gt = data_batch['gt']
        GT_img, LR_img, ker_map = self.forward_dummy(gt)
        outputs = self(LR_img, GT_img, ker_map, test_mode=False)
