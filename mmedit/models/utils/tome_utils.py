...
def add_tome_cfg_hook(model: torch.nn.Module):
    """Add a forward pre hook to get the image size
...
def build_mmedit_tomesd_block(block_class: Type[torch.nn.Module]) -> Type[torch.nn.Module]:
    """Make a patched class for a mmedit model. This patch applies ToMe to the forward
    ...
    class ToMeBlock(block_class):
        ...
        def forward(
            ...
            # ->(1) ToMeBlock
            m_a, m_c, m_m, u_a, u_c, u_m = build_merge(hidden_states, self._tome_info)
...
def isinstance_str(x: object, cls_name: str):
    """Checks whether `x` has any class *named* `cls_name`
    in its ancestry. Doesn't require access to the class's
    implementation.
...
def mps_gather_workaround(input, dim, index):
    """Gather function specific for `mps` backend
...
def bipartite_soft_matching_random2d(metric: torch.Tensor,
    ...
    """Partitions the tokens into src and dst and merges r tokens from src to dst, dst
    tokens are partitioned by choosing one randomy in each (`sx`, `sy`) region. More
    details refer to `Token Merging: Your ViT But Faster`, paper link:
    <https://arxiv.org/abs/2210.09461>`_ # noqa
...
def build_merge(x: torch.Tensor, tome_info: Dict[str, Any]) -> Tuple[Callable, ...]:
