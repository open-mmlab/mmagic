...
def calculate_weights_indices(in_length, out_length, scale, kernel, kernel_width, antialiasing, device):
    ...
    weights = weights.contiguous().to(device)
...
def imresize(img, scale, antialiasing=True):
    ...
    Args:
        img (Tensor):
            Tensor: Input image with shape (n, c, h, w), [0, 1] range.
    ...
    Returns:
        Tensor: Output image with shape (n, c, h, w), [0, 1] range, w/o round.
    """
    squeeze_flag = False
    if img.ndim == 3:
        img = img.unsqueeze(1)
    ...
    out_1 = torch.empty(in_n, in_c, out_h, in_w, device=img.device)
    ...
    out_2 = torch.empty(in_n, in_c, out_h, out_w, device=img.device)
