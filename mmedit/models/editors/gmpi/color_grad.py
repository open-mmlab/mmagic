...
def rand_hex_color(num=1):
    ...
    """
    colors = [
        RGB_to_hex([x * 255 for x in np.random.rand(3)]) for i in range(num)
    ]
    if num == 1:
        return colors[0]
    else:
...
def polylinear_gradient(colors, n):
    """returns a list of colors forming linear gradients between all sequential
    pairs of colors.
...
def bezier_gradient(colors, n_out=100):
    """Returns a "bezier gradient" dictionary using a given list of colors as
    control points.

    Dictionary also contains control colors/points.
    """
