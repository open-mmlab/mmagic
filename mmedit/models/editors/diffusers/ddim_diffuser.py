...
class DDIMDiffuser:
    """```DDIMDiffuser``` support the diffusion and reverse process formulated
    ...
    def __init__(
        ...
        # At every step in ddim, we are looking into the previous
        # alphas_cumprod For the final step, there is no previous
        # alphas_cumprod because we are already at 0 `set_alpha_to_one`
        # decides whether we set this paratemer simply to one or
    ...
    def step(
        ...
        variance = self._get_variance(
            timestep, prev_timestep, predicted_variance=predicted_variance)
        std_dev_t = eta * variance**(0.5)

        if use_clipped_model_output:
            # the model_output is always re-derived from the clipped x_0 in
            # Glide
