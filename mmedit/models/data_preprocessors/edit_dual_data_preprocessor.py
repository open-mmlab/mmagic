...
from .edit_data_preprocessor import stack_batch


@MODELS.register_module()
class EditDualDataPreprocessor(BaseDataPreprocessor):
    ...
    def forward(
        ...
        self.norm_input_flag = (inputs['img_l'][0].max() <= 1)

        # Normalization.
        inputs_l = [(_input - self.input_mean) / self.input_std
                    for _input in inputs['img_l']]
        inputs_r = [(_input - self.input_mean) / self.input_std
                    for _input in inputs['img_r']]
        inputs = [
            torch.cat((inputs_l[i], inputs_r[i]), 0)
