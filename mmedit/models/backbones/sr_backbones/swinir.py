...
from mmedit.models.utils.transformer import PatchEmbed, PatchUnEmbed
...
class WindowMSA(nn.Module):
    r""" Window based multi-head self attention (W-MSA) module with relative p
        osition bias.
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, input_resolution, dim, norm_cfg=dict(type='LN')):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
        self.reduction = nn.Linear(4 * dim, 2 * dim, bias=False)
        if norm_cfg is not None:
            self.norm = build_norm_layer(norm_cfg, 4 * dim)[1]
        else:
            self.norm = norm_cfg
    ...
    def __init__(self,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(
                ...
                drop_rate=drop,
                attn_drop=attn_drop,
                drop_path_rate=drop_path_rate[i] if isinstance(
        ...
        if downsample is not None:
            self.downsample = downsample(
                input_resolution, dim=dim, norm_cfg=norm_cfg)
        else:
...
class UpsampleOneStep(nn.Sequential):
    """UpsampleOneStep module
       (the difference with Upsample is that
        it always only has 1conv + 1pixelshuffle)
...
class SwinIR(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_first = nn.Conv2d(num_in_ch, embed_dims, 3, 1, 1)
        ...
        for i_layer in range(self.num_layers):
            layer = RSTB(
                ...
                drop_path_rate=dpr[sum(depths[:i_layer]):sum(
        ...
        if resi_connection == '1conv':
            ...
            self.conv_after_body = nn.Sequential(
                nn.Conv2d(embed_dims, embed_dims // 4, 3, 1, 1),
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            self.conv_before_upsample = nn.Sequential(
                nn.Conv2d(embed_dims, num_feat, 3, 1, 1),
                nn.LeakyReLU(inplace=True))
            self.upsample = Upsample(upscale_factor, num_feat)
            self.conv_last = nn.Conv2d(num_feat, num_out_ch, 3, 1, 1)
        elif self.upsampler == 'pixelshuffledirect':
            # for lightweight SR (to save parameters)
            self.upsample = UpsampleOneStep(upscale_factor, embed_dims,
                                            num_out_ch,
            ...
            self.conv_last = nn.Conv2d(embed_dims, num_out_ch, 3, 1, 1)
