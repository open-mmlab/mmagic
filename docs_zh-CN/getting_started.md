...
如果 `--imshow` 被指定，这个脚本也能通过 opencv 展示图片。比如：
...
如果 `--unpaired_path` 被指定了（适用于 CycleGAN），模型将会惊醒图片到图片的变换。如果 `--imshow` 被指定，这个脚本也能通过 opencv 展示图片。比如：
...
所有的输出（日志文件和模型文件）将会被存储到工作目录下面，工作目录可以通过 `work_dir` 参数在配置文件中指定。
...
- `--resume-from ${CHECKPOINT_FILE}`: 从之前的模型文件中重启训练。
...
如果你启动训练任务在 slurm 系统上，你需要修改配置文件来设定不同的信息交互端口：
...
之后你可以启动两个任务用修改后的 `config1.py` 和 `config2.py`：
...
**注意**：这个工具目前还是出试验阶段，我们不能确保这个数值是正确的。当你需要在技术报告或者论文中使用时，请谨慎检查这个数值：
...
在你上传模型到 AWS 之前，你需要：
(1) 将模型的权重转换为 CPU 向量, (2) 删除优化器的状态量 and
