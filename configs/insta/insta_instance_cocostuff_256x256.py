...
model = dict(
    ...
    loss=dict(type='HuberLoss', delta=1. / ab_norm),
    instance_model=dict(
...
train_pipeline = [
    ...
    dict(type='LoadBboxFromFile', key='instance', stage='instance'),
    dict(
        type='GenGrayColorPil', stage='instance', keys=['rgb_img',
                                                        'gray_img']),
    dict(
        type='Resize',
    ...
    dict(
        type='Collect',
        keys=['instance', 'rgb_img', 'gray_img'],
        meta_keys=['gt_img_path']),
    dict(type='ImageToTensor', keys=['instance', 'rgb_img', 'gray_img'])
]

dataset_type = 'COCOStuff_Instance_Dataset'
data_root = '/mnt/cache/share_data/zhangwenwei/data/coco/train2017'

npz_root = '/mnt/cache/yuruoning.vendor/data'
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file=f'{npz_root}/img_list.txt',
        data_prefix=data_root,
        npz_prefix=f'{npz_root}/train_bbox/train2017_bbox',
