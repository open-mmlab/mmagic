...
config = "projects/flow_style_vton/configs/flow_style_vton_PFAFN_epoch_101.py"
...
parser.add_argument('--dataroot', type=str, default="projects/flow_style_vton/VITON_test") 
parser.add_argument('--test_pairs', type=str, default="projects/flow_style_vton/test_pairs.txt")
...
for i, data in enumerate(tqdm(dataset)):
    ...
    save_image(
        p_tryon,
        os.path.join(opt.output_dir, "our_t_results", data['p_name'][0]),
        nrow=int(1),
        normalize=True,
        value_range=(-1,1)
    )
    save_image(
            combine,
            os.path.join(opt.output_dir, 'im_gar_flow_wg', data['p_name'][0]),
            nrow=int(1),
            normalize=True,
            range=(-1,1),
    )
