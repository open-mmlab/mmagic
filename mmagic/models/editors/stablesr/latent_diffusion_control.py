...
from ..controlnet.controlnet_utils import change_base_model
...
class ControlLatentDiffusion(StableDiffusion):
    ...
    # TODO: from ControlLatentDiffusionImg2Img
    ...
    def prepare_latents(self,
        ...
        # self.vae.encode [2,3,512,512] -> feat_lq
        if isinstance(generator, list):
            ...
            if image.shape[-1] == 512:
                init_latents = torch.rand(2, 4, 64, 64)
            else:
    ...
    def infer(
        ...
        # 8. Post-processing
        enc_fea_lq_1 = torch.rand(2, 256, 256, 256)
        enc_fea_lq_2 = torch.rand(2, 512, 128, 128)
        enc_fea_lq = [enc_fea_lq_1, enc_fea_lq_2]
        image = self.decode_latents(latents.to(img_dtype), enc_fea_lq)
    ...
    def decode_latents(self, latents, enc_fea_lq):
        ...
        # should accept enc_fea_lq as decode input
        latents = 1 / 0.18215 * latents
        if hasattr(self.vae, 'module'):
            image = self.vae.module.decode(latents)['sample']
        else:
            # image = self.vae.decode(latents)['sample']
            image = self.vae.decode(latents, enc_fea_lq)['sample']
