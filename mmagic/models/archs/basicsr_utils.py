...
class BlockSplitting(nn.Module):
    ...
    def forward(self, image):
        ...
        return image_transposed.contiguous().view(batch_size, -1, self.k,
                                                  self.k)
...
class BlockMerging(nn.Module):
    ...
    def forward(self, patches, height, width):
        ...
        image_reshaped = patches.view(batch_size, height // k, width // k, k,
                                      k)
        image_transposed = image_reshaped.permute(0, 1, 3, 2, 4)
...
class YCbCr2RGBJpeg(nn.Module):
    ...
    def __init__(self):
        ...
        matrix = np.array([[1., 0., 1.402], [1, -0.344136, -0.714136],
                           [1, 1.772, 0]],
                          dtype=np.float32).T
        self.shift = nn.Parameter(torch.tensor([0, -128., -128.]))
        self.matrix = nn.Parameter(torch.from_numpy(matrix))
...
class DeCompressJpeg(nn.Module):
    ...
    def forward(self, y, cb, cr, imgh, imgw, factor=1):
        ...
        for k in components.keys():
            ...
            #
        image = self.chroma(components['y'], components['cb'],
                            components['cr'])
...
class DiffJPEG(nn.Module):
    """This JPEG algorithm result is slightly different from cv2. DiffJPEG
    supports batch processing.

    Args:
        differentiable(bool):
        If True, uses custom differentiable rounding function,
        if False, uses standard torch.round
    """
