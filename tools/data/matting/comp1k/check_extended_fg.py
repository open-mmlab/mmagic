...
imgs = [os.path.splitext(os.path.basename(x))[0] for x in glob.glob(f'{folder}/fg/*.jpg')]

print('max,avg,img')
for name in imgs:
    alpha = cv2.imread(f'{folder}/alpha/{name}.jpg', cv2.IMREAD_GRAYSCALE).astype(np.float32)[..., None] / 255
