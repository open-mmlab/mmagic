...
def update_model_zoo():
    ...
    for f in tqdm(files, desc='update model zoo'):
        ...
        paperlinks = {}
        for _, p in papers:
            paper_link = osp.join(github_link, 'configs', basename(dirname(f)), 'README.md')
            # print(p, paper_link)
            paperlinks[p] = ' '.join(
...
"""
    ...
    allpapers = func.reduce(lambda a, b: a.union(b), [p for p, _, _, _, _ in stats])
    allconfigs = func.reduce(lambda a, b: a.union(b), [c for _, c, _, _, _ in stats])
    allckpts = func.reduce(lambda a, b: a.union(b), [c for _, _, c, _, _ in stats])
    alltasks = func.reduce(lambda a, b: a.union(b), [t for _, _, _, t, _ in stats])
    task_desc = '\n    - '.join(list(alltasks))
